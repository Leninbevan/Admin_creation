...
import { edituserDetails } from './service/action';
...
export const Edituser = () => {
    ...
    const editData = addedDetails.find((item, index) => {
    ...
    const formik = useFormik({
        initialValues: {
            name: editData.name,
            image: editData.profile_picture,
            job: editData.job,
        ...
        onSubmit: (values) => {
            console.log(values);
            dispatch(edituserDetails(values, userId.id, token));
            ...
            <Box sx={{ bgcolor: 'black', borderRadius: '4%', width: '30%' }}>
                <form onSubmit={formik.handleSubmit} >
                    <Box sx={{ display: 'flex', flexDirection: 'column', padding: 5 }}>
                        <Box className="head" style={{ color: 'white', textAlign: 'center' }}>Edit User</Box>
                        <TextField sx={{
